...
exports.deleteTheses = async (req, res) => {
    try {
        ...
        const requests = await RequestsProgress.find({ ThesisID: theses._id });
        if (requests && requests.length > 0) {
            await Document.deleteMany({ RequestID: { $in: requests.map((r) => r._id) } });

            await RequestsProgress.deleteMany({ ThesisID: theses._id });
        }

        await Theses.findByIdAndDelete(theses._id);
